...
- Default model `gemini-2.0-flash`; expose helpers for text generation and embeddings (`gemini-embedding-exp` for embedding generation — see LAU-EMBED-01).
...
- [ ] Embedding calls use `gemini-embedding-exp`
...
**Notes**
Only upgrade off flash with a documented reason (coding standard). Embeddings use `gemini-embedding-exp` specifically.
...
- Generate embedding via Gemini API (`gemini-embedding-exp`). Store as a Firestore `VectorValue` field `embedding` on the entry doc.
- Embedding dim per the model output.

**Acceptance criteria**
- [ ] Every newly analyzed entry has an `embedding` VectorValue field
- [ ] Embedding is generated via Gemini API (`gemini-embedding-exp`)
...
**Notes**
Vertex AI is not used anywhere in the stack. Embeddings are generated by `gemini-embedding-exp` and stored in Firestore. All search and RAG uses Firestore KNN `findNearest()`.
...
**Implementation Details**
- Embed the query via Gemini (`gemini-embedding-exp`), call `findNearest()` (LAU-EMBED-03), render ranked results.
...
**XPRIZE relevance**
Category Impact — the "living map of your mind" made literal.
